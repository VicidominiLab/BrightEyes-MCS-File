...
def estimate_channel_skew(
    ...
    reference_hist : ndarray or None
        ...
        `reference_hist[:, reference_channel]`. When ``None``, the last
    ...
    reference_channel : int or None
        ...
        ``None``, the middle channel (or last channel for single-array
        references) is used.
    upsampling : int
    ...
    errors : ndarray
        Placeholder array of the same length filled with ``np.nan``. A
        more sophisticated estimator could populate uncertainties.
    meta : dict
    ...
    if reference_hist is None:
        # use last channel of data as reference by default
    ...
    # FFT of reference (single column)
    ref_vec = ref[:, 0]
    F_ref = np.fft.fft(ref_vec)
